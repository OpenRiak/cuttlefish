...
%% -------------------------------------------------------------------
%%
%% For all your .conf parsing needs.
%%
...
included_file_test() ->
    Conf = conf_parse:file(cuttlefish_test_util:test_file("include_file.conf")),
    ?assertEqual([
            {include,<<"riak.conf">>}
        ], Conf),
    ok.
...
included_dir_test() ->
    Conf = conf_parse:file(cuttlefish_test_util:test_file("include_dir.conf")),
    ?assertEqual([
            {include,<<"conf.d/*.conf">>}
        ], Conf),
    ok.
