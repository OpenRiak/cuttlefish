...
%% -------------------------------------------------------------------
%%
%% @doc Used by sh scripts to parse configs
%%
...
main(Args) ->
    ...
    ok = logger:update_handler_config(?LOGGER_HANDLER, LC1),
...
load_conf(ParsedArgs) ->
    ...
    case cuttlefish_conf:files(ConfFiles) of
        {errorlist, Errors} ->
            _ = [ ?LOG_ERROR(cuttlefish_error:xlate(E)) ||
                    {error, E} <- Errors],
            stop_deactivate(),
            {errorlist, Errors};
        GoodConf ->
...
delete([File|Files], MaxHistory) ->
    case file:delete(File) of
        ok -> ok;
        {error, Reason} ->
            ?LOG_ERROR("Could not delete ~ts, ~tp", [File, Reason])
    end,
...
maybe_log_file_error(Filename, {error, Reason}) ->
    ?LOG_ERROR("Error writing ~ts: ~ts", [Filename, file:format_error(Reason)]).
