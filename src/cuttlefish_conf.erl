...
generate_element(MappingRecord) ->
    ...
    case Level of
        basic -> ok;
        Level ->
            ?LOG_WARNING("{level, ~p} has been deprecated. Use 'hidden' or '{hidden, true}'", [Level])
    end,
...
included_dir_test() ->
    Conf = file(cuttlefish_test_util:test_file("include_dir.conf")),
    ?assertEqual(lists:sort([
            {["anti_entropy"],"debug"},
            {["log","error","file"],"/var/log/error.log"},
            {["log","console","file"],"/var/log/console.log"},
...
included_value_test() ->
    Conf = file(cuttlefish_test_util:test_file("included_value.conf")),
    ?assertEqual(lists:sort([
             {["value1"], "42"},
             {["value2"], "43"},
             {["value3"], "42"},
